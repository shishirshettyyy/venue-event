      ...
      <form onSubmit={handleSubmit} noValidate className="auth-card mt-8 space-y-5">
        ...
        <div>
          ...
          <div className="mt-2 flex gap-3">
            <button type="button" onClick={() => setRole('attendee')}
              className={`chip flex-1 justify-center ${role === 'attendee' ? 'chip-active' : ''}`}>
              Browse &amp; book events
            </button>
            <button type="button" onClick={() => setRole('organizer')}
              className={`chip flex-1 justify-center ${role === 'organizer' ? 'chip-active' : ''}`}>
              Organise events
            </button>
          </div>
        </div>
