...
export default function Payment() {
  ...
  // Booking.jsx passes: { bookingId, seats, total, event }
  ...
  const eventTitle = draft.event?.title || 'Event'
  const seats = draft.seats || 1

  const qrPayload = encodeURIComponent(
    `upi://pay?pa=venue.tickets@upi&pn=Venue&am=${total}&cu=INR&tn=${draft.bookingId}`
  )
  ...
  async function completePayment(e) {
    ...
    try {
      await api.post('/payments', {
        ...
        method: method === 'qr' ? 'upi' : 'card',
        upiId: method === 'qr' ? 'venue.tickets@upi' : undefined,
        cardDetails: method === 'card'
          ? { cardNumber: cardNumber.replace(/\s/g, ''), cardName, cardExpiry, cardCvv }
          : undefined,
      })
      navigate(`/bookings/${draft.bookingId}/confirmation`)
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-10 sm:px-8">
      ...
      <p className="mt-1 text-sm text-muted">{eventTitle} · {seats} seat{seats > 1 ? 's' : ''}</p>
      ...
      <div className="mt-8 grid gap-8 sm:grid-cols-[1fr_280px]">
        <div>
          <div className="flex gap-2">
            <button type="button" onClick={() => { setMethod('qr'); setErrors({}) }}
          ...
          <form onSubmit={completePayment} noValidate className="mt-6 rounded-2xl border border-line bg-surface p-6">
            {method === 'qr' ? (
              <div className="flex flex-col items-center text-center">
                <img src={qrSrc} alt="Scan this QR code with any UPI app to pay"
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input id="cardName" type="text" value={cardName}
                    onChange={(e) => setCardName(e.target.value)} placeholder="As printed on the card"
                ...
                <div>
                  ...
                  <input id="cardNumber" type="text" inputMode="numeric" value={cardNumber}
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                    ...
                    <input id="expiry" type="text" inputMode="numeric" value={cardExpiry}
                      onChange={(e) => setCardExpiry(formatExpiry(e.target.value))} placeholder="MM/YY"
                  ...
                  <div className="flex-1">
                    ...
                    <input id="cvv" type="password" inputMode="numeric" value={cardCvv}
            ...
            <p className="mt-3 text-center text-xs text-muted">This is a demo — no real payment is processed</p>
          </form>
        </div>
